...
describe("base bonus trigger accounting", () => {
  it.each([
    { seed: 68, scatterCount: 4, freeSpinsAwarded: 10 },
    { seed: 1222, scatterCount: 5, freeSpinsAwarded: 15 },
    { seed: 8399, scatterCount: 6, freeSpinsAwarded: 20 },
    { seed: 200795, scatterCount: 7, freeSpinsAwarded: 25 },
  ...
  it("uses the final settled board after a tumble/refill trigger", () => {
    const result = playSpin(100, new SeededRNG(462));
    ...
    expect(scatterPositions(finalBoard)).toEqual(["0:2", "0:3", "0:4", "1:2"]);
  });
});
