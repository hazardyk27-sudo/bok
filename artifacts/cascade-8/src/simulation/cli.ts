...
async function main() {
  ...
  console.table({
    ...
    "Bonus Frequency": report.bonusFrequency,
     "Free Spin Count": report.freeSpinCount,
     "Free Spin Hit Rate": `${report.freeSpinHitRate}%`,
     "Free Spin Initial Board >= 1 Core": `${report.freeSpinInitialAtLeastOneCoreRate}%`,
     "Free Spin Initial Board >= 2 Cores": `${report.freeSpinInitialAtLeastTwoCoreRate}%`,
    ...
    "Max Observed Win": `${report.maxObservedWinMultiplier}x`,
     ...
     "Chain Rate": `${report.chainRate}%`,
      ...
      "Actual Second = First": `${report.observedSecondSameProbability}%`,
       "Configured Third Direct Copy Branch": `${report.configuredThirdCopyBranchProbability}%`,
       "Observed Third Direct Copy Branch": `${report.observedThirdCopyBranchProbability}%`,
