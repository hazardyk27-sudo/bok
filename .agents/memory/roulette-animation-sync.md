...
**How to apply:** Offset repeating spin animations by elapsed phase time, then use the authoritative winning number to drive the final rotor alignment and ball drop. Keep the ball’s pocket-drop radius aligned with the visual pocket-ring radius, and counter-rotate final labels when the rotor settles at a non-zero angle. When CSS custom properties contain `clamp()` values, resolve them through layout rather than parsing the first numeric token, or responsive animation will use the wrong radius.