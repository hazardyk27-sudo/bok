...
export function simulate(spins: number, seed: string, betCents = 100, onProgress?: (completed: number) => void): SimulationReport {
  ...
  for (let index = 0; index < spins; index += 1) {
    ...
    for (const freeSpin of result.freeSpins) if (freeSpin.retriggered > 0) retriggeredFreeSpins += freeSpin.retriggered;
    ...
    if (result.tumbles.length > 1 || result.freeSpins.some((freeSpin) => freeSpin.tumbles.length > 1)) chainedSpinCount += 1;
  ...
  const runTotal = (runLengthDistribution["1"] ?? 0) + (runLengthDistribution["2"] ?? 0);
  return {
    seed, totalSpins: spins, betCents, totalBetCents, totalReturnCents,
    ...
    hitRate: percent(hitCount), zeroWinRate: percent(spins - hitCount),
    bonusTriggerCount, bonusFrequency: bonusTriggerCount ? `1 in ${(spins / bonusTriggerCount).toFixed(2)}` : "none observed",
    ...
    averageTumblesPerPaidSpin: Number((tumbleCount / spins).toFixed(4)), maximumTumblesSeen: maxTumbles,
    ...
    medianWinMultiplier: Number(median(wins).toFixed(4)), maxObservedWinMultiplier: Number(maxObservedWinMultiplier.toFixed(4)),
    ...
    coreValueDistribution, histogram,
  };
}
