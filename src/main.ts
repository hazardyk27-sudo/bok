    ...
    <main class="game-layout">
      ...
      <aside class="side-panel">
        ...
        <div class="legend-card">
          ...
          ${NORMAL_SYMBOLS.map((symbol) => `<div class="legend-row"><span class="legend-icon" style="color:${symbol.colorHex}">${symbol.icon}</span><span>${symbol.id}</span><b>${symbol.weight.toFixed(1)}%</b></div>`).join("")}
        </div>
...
function showModal(name: string | null) {
  ...
  if (name === "settings") {
    ...
    modalRoot.innerHTML = `<div class="modal-backdrop"><section class="modal-card info-modal"><button class="modal-close" data-close>×</button><div class="modal-kicker">CASCADE 8 // FIELD GUIDE</div><h2>How to play</h2><p class="modal-lead">Match 8 or more of a symbol anywhere on the field. Winning symbols burst, the field falls, and fresh symbols tumble in.</p>
      ...
      <div class="paytable"><div class="paytable-head"><span>SYMBOL</span><span>8 — 9</span><span>10 — 11</span><span>12+</span></div>${NORMAL_SYMBOLS.map((symbol) => { const paytable = PAYTABLE[symbol.id as keyof typeof PAYTABLE]; return `<div class="paytable-row"><span style="color:${symbol.colorHex}">${symbol.icon} ${symbol.id}</span><span>${paytable[0].multiplier}x</span><span>${paytable[1].multiplier}x</span><span>${paytable[2].multiplier}x</span></div>`; }).join("")}</div>
